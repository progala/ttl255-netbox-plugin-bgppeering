...
NAME = os.getenv("IMAGE_NAME", "ttl255-netbox-plugin-bgppeering")
PWD = os.getcwd()
...
@task
def makemigrations(
    context, app_name=BUILD_NAME, name="", netbox_ver=NETBOX_VER, python_ver=PYTHON_VER
):
...
def unittest(context, netbox_ver=NETBOX_VER, python_ver=PYTHON_VER):
    ...
    context.run(
        f'{docker} sh -c "python manage.py test {BUILD_NAME}"',
        env={"NETBOX_VER": netbox_ver, "PYTHON_VER": python_ver},
        pty=True,
    )
